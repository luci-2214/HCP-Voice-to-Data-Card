...
METRICS_PROMPT = """You are a healthcare programmatic advertising analytics AI at Doceree.
Based on the HCP persona below, generate realistic campaign dashboard metrics.
...
{{
  "hcp_count": "<number like 8.5k or 22.1k>",
  "hcp_delta": "▲ +<number>% vs last quarter",
  "sentiment": "<Positive or Neutral or Negative>",
  "sentiment_detail": "Confidence: <High or Medium or Low>",
  "ctr": "<number like 1.8% or 3.2%>",
  "ctr_delta": "▲ +<number>% vs benchmark",
  "accuracy": <integer 60-95>
}}"""
                    ...
                    if metrics_raw:
                        try:
                            ...
                            if json_match:
                                ...
                                st.session_state["metrics"] = {
                                    "hcp_count": metrics_data.get("hcp_count", "14.2k"),
                                    "hcp_delta": metrics_data.get("hcp_delta", "▲ +5% vs last quarter"),
                                    "sentiment": metrics_data.get("sentiment", "Positive"),
                                    "sentiment_detail": metrics_data.get("sentiment_detail", "Confidence: High"),
                                    "ctr": metrics_data.get("ctr", "2.4%"),
                                    "ctr_delta": metrics_data.get("ctr_delta", "▲ +0.2% vs benchmark"),
                                    "accuracy": accuracy_val,
                                    "accuracy_ref": max(accuracy_val - 7, 50),
                                }
                                st.rerun()  # Rerun to update the metrics cards at the top
                        except (json.JSONDecodeError, ValueError):
                            pass  # Keep default metrics if parsing fails
                else:
...
# Plotly Gauge Chart — uses dynamic accuracy from AI
gauge_value = st.session_state["metrics"]["accuracy"]
gauge_ref = st.session_state["metrics"]["accuracy_ref"]
